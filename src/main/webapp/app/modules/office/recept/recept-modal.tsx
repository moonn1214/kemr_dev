...
export const ReceptModal = (props: IReceptModalProps) => {
  ...
  const confirmRecept = () => {
    const kemrPatient = kemrPatients.find(kemrPatient => kemrPatient.id.toString() === props.kemrPatientId.toString());
    const kemrPatientSocialSecurityNo = props.kemrPatientSocialSecurityNo.replace(/[^0-9]/g, "");

    // 기존 환자 정보 수정이 있을 경우 환자 정보 수정 액션 호출
    if (props.kemrPatientName !== kemrPatient.kemrPatientName || 
        props.kemrPatientSocialSecurityNo !== kemrPatient.kemrPatientSocialSecurityNo ||
      ...
      const patientEntity = {
        ...
        kemrPatientSex: getPatientSex(kemrPatientSocialSecurityNo),
        // (수정)생일 삭제되면 삭제
        kemrPatientBirthday: getPatientBirthday(kemrPatientSocialSecurityNo),
        kemrPatientRegistrationDate: kemrPatient.kemrPatientRegistrationDate,
        kemrPatientSocialSecurityNo: kemrPatientSocialSecurityNo.substring(0, 6) + "-" + kemrPatientSocialSecurityNo.substring(6),
    ...
    // 정보 수정 없는 경우 접수
      ...
      const entity = {
        kemrPatient: kemrPatients.find(kemrPatient => kemrPatient.id.toString() === props.kemrPatientId.toString()),
        kemrMedicalTreatmentNurseMessage: props.kemrNurseMessage,
        kemrDoctor: kemrDoctors.find(kemrDoctor => kemrDoctor.id.toString() === props.kemrDoctorId.toString()),
  ...
  const confirmPatientRecept = () => {
    const patientEntity = {
      kemrPatientName: props.kemrPatientName,
      kemrPatientSex: getPatientSex(props.kemrPatientSocialSecurityNo),
      // (수정)생일 삭제되면 삭제
      kemrPatientBirthday: getPatientBirthday(props.kemrPatientSocialSecurityNo),
      kemrPatientRegistrationDate: new Date().toISOString().replace(/\..*/, '') + 'Z',
      kemrPatientSocialSecurityNo: props.kemrPatientSocialSecurityNo.substring(0, 6) + "-" + props.kemrPatientSocialSecurityNo.substring(6),
      kemrPatientQualificationCheck: props.kemrPatientQualificationCheck,
      kemrPatientAddress: props.kemrPatientAddress,
      kemrPatientCellphone: props.kemrPatientCellphone.replace(/(^02|^0505|^1[0-9]{3}|^0[0-9]{2})([0-9]+)?([0-9]{4})$/,"$1-$2-$3")
    };
